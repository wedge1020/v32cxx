...
# 41, 42, 43, 44, 45, 46, 47, 48, and 49, which genuinely do define their
...
# real Vircon32 toolchain, not just transpiled; sample22 through 30, 32,
# 33, and 34 already had one; 35 through 49 -- base-class constructor
# delegation, member-field initializers, and bitwise operators/switch,
# several later rounds -- also define their own, even though 36, 38, 42,
# 45, and 49 are themselves deliberately invalid -- see below). sample31
...
# complete program at all. sample36/38/42/45/49 are ALSO deliberately
# invalid (an unknown name, a class-typed field, a field named twice, a
# base with no zero-arg constructor, and `continue` inside a switch with
# no enclosing loop, respectively) but do NOT get `-c` -- they already
# define their own `main`, same as 35, so `-c` would be redundant, not
# wrong, but left off for consistency with how 35 itself is invoked.
# sample37 was ALSO deliberately invalid once (the very case sample38
# now tests -- a member-initializer list naming an ordinary field, "not
# yet supported" at the time), repurposed into a real, passing test once
...
# sample24/32 needed real, one-line fixes the SAME round implicit
# base-class construction shipped (see docs/DESIGN_NOTES.md) -- both had
# a derived constructor that never explicitly delegated to a base with
# no zero-arg constructor of its own, which was always invalid real
# C++, just never caught until this project's own
